...
class Notification(BaseModel):
    ...
    read_at: datetime | None = Field(default=None, description="Дата и время прочтения уведомления")
    category: str | None = Field(default=None, description="Категория уведомления")
    confidence: float | None = Field(default=None, description="Оценка соответствия контента категории уведомления")
    processing_status: ProcessingStatus = Field(..., description="Статус обработки уведомления")
...
class NotificationFilters(BaseModel):
    ...
    title_strict: bool = Field(default=True, description="Если True поиск полного соответствия, иначе поиск подстроки")
    text: str | None = Field(default=None, description="Текст уведомления")
    created_at_start: datetime | None = Field(default=None, description="Время и дата начала создания уведомления")
    created_at_end: datetime | None = Field(default=None, description="Время и дата завершения создания уведомления")
    readed_at_start: datetime | None = Field(default=None, description="Время и дата начала прочтения уведомления")
    readed_at_end: datetime | None = Field(default=None, description="Время и дата завершения прочтения уведомления")
    category: str | None = Field(default=None, description="Категория уведомления")
    category_strict: bool = Field(default=False, description="Если True поиск полного соответствия, иначе поиск подстроки")
    confidence_start: float | None = Field(default=None, description="Нижний порог оценки соответствия контента категории уведомления")
    confidence_end: float | None = Field(default=None, description="Верхний порог оценки соответствия контента категории уведомления")
    processing_status: ProcessingStatus | None = Field(default=None, description="Статус обработки уведомления")
    limit: int = Field(default=10, description="Лимит записей в запросе")
    offset: int = Field(default=0, description="Смещение по записям")
    is_read: bool | None = Field(default=None, description="Отобразить только прочтенные уведомления")
...
class NotificationsList(BaseModel):
    ...
    model_config = ConfigDict(from_attributes=True)
    data: Sequence[Notification] = Field(default=[], description="Список найденных уведомлений")
